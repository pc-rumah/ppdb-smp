...
@section('content')
    <main id="main" class="main">
        <div class="pagetitle">
            <h4>Kelola Data Pengumuman Pondok</h4>
            <nav>
                <ol class="breadcrumb">
            ...
            <div class="col-xl-12">
                <div class="card  bg-info bg-gradient">
                    <div class="card-body pt-3">
                        <div class="row">
                            <div class="col-lg-8">
                                <a href="{{ route('pengumumanpondok.create') }}" type="button" class="btn btn-light">Tambah
                                    Data</a>
                            </div>
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody>
                                            @forelse ($data as $item)
                                                <tr>
                                                    <th scope="row">{{ $data->firstItem() + $loop->index }}
                                                    </th>
                                                    <td>{{ $item->judul }}</td>
                                                    ...
                                                    <td class="d-flex gap-2">
                                                        @if (Auth::user()->hasRole('master-admin'))
                                                            @if ($item->status == 'pending')
                                                                <form
                                                                    action="{{ route('pengumumanpondok.approve', $item->id) }}"
                                                                ...
                                                                <form
                                                                    action="{{ route('pengumumanpondok.reject', $item->id) }}"
                                                                    method="POST" class="d-inline">
                                                                    @csrf
                                                                    <button type="submit"
                                                            ...
                                                            @else
                                                                <form
                                                                    action="{{ route('pengumumanpondok.approveDelete', $item->id) }}"
                                                                    method="POST" class="d-inline">
                                                                    @csrf
                                                                    <button type="submit"
                                                                        class="btn btn-success btn-sm">Approve</button>
                                                                ...
                                                                <form
                                                                    action="{{ route('pengumumanpondok.rejectDelete', $item->id) }}"
                                                                    method="POST" class="d-inline">
                                                                    @csrf
                                                                    <button type="submit"
                                                                        class="btn btn-danger btn-sm">Reject</button>
                                                            ...
                                                            @endif
                                                        @else
                                                            <a href="{{ route('pengumumanpondok.edit', $item) }}"
